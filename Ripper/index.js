...
async function login(page)
{
  ...
  let values = config.values
  await page.type('input[type="email"]', values.emailInput);
  await page.type('input[type="password"]', values.passwordInput);
  let imagePath = screenshots_directory + '/' + 'email' + '.png';
  await page.screenshot({path: imagePath, fullPage: true});
  await page.click('button[type="submit"]');
  await new Promise(r => setTimeout(r, 4000));
  console.log(`Clicked "login"`)
  imagePath = screenshots_directory + '/' + 'afterLogin' + '.png';
  await page.screenshot({path: imagePath, fullPage: true});
  return;
}
//Fix URL strings
